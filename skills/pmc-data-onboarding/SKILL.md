...
#### 4. 业务参数预期（决定 ods_params 预设）

| 提问 | 选项 | 对 PMC 的影响 |
|------|------|------|
| 预期 S/A/B/C/N 货盘分级阈值？（如按 Pareto 贡献度分） | 有想法 / 用默认 / 后续再定 | 有明确预期则写入 ods_params 覆盖默认值 |
| 安全库存天数有偏好吗？ | A类30天 B类45天 / 要更激进 / 要更保守 / 用默认 | 写入 P1/P2 参数 |
| 采购提前期大概多少天？ | <15天 / 15-30天 / 30-60天 / >60天 | 写入 P4 采购/发货周期段 |
| 期望几天跑一次分析？ | 每天 / 每周 / 每月 | 决定 cron 频率 |
...
║  • 安全库存：A类30天 / B类45天（客户偏好保守）       ║
║  • 补货周期：快船15天（覆盖90%）                     ║
║  • ods_params 预设：写入P1/P2/P4                    ║
║  • 接入方式：Excel导入 + 每周cron                   ║
║  • 需前置处理：组合装拆解（阶段C）                   ║
╚═══════════════════════════════════════════════════╝
```
