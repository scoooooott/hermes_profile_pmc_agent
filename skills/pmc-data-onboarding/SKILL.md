...
version: 1.0.0
description: "PMC 新客户数据接入向导：分 6 阶段逐步引导客户完成客户画像 → 板块发现 → 数据探查 → 编码归一化 → 管道配置 → 端到端验证。PMC Agent 加载此 Skill 后按流程推进，确认一步再走下一步。"
metadata:
  triggers:
    - "新客户接入"
    - "数据接入"
...
在进入数据接入前，先建立客户画像。这决定了后续所有阶段的默认配置——不同业务模式对应不同的供应链模型，参数预设完全不同。

**重要**：此阶段目的不是「筛选」客户（判断是否值得接），而是**自适应配置**——了解客户的模式后，PMC 自动调整参数预设、跳过不适用的数据板块、给出适合该模式的分析重点。

### 画像维度

以下提问覆盖了 PMC 需要了解的核心信息。不是必答题——客户答不上来的跳过去，有答案的记录下来，用于后续阶段智能调参。

#### 1. 商业模式（决定供应链形态）

| 提问 | 选项 | 对 PMC 的影响 |
|------|------|------|
| 主要的业务模式是？ | 精品/品牌、精铺、铺货、工贸一体 | 精品偏品牌运营，库存深度大；铺货 SKU 多但无国内仓；工贸一体有生产周期数据 |
| 是自己品牌还是代理/经销？ | 自有品牌 / 代理经销 / 混合 | 自有品牌可对接生产周期参数；代理无生产数据 |
| 大致 SKU 数量？ | <100 / 100-1000 / 1000-10000 / >10000 | 影响 ods_skus 行数和加权计算性能 |
| 年 GMV 大概量级？ | <500万 / 500万-5000万 / 5000万-2亿 / >2亿 | 影响数据量和分析频率 |

#### 2. 平台组合（决定仓库体系）

| 提问 | 选项 | 对 PMC 的影响 |
|------|------|------|
| 在哪些平台卖？ | 亚马逊 / Temu / SHEIN / TikTok Shop / 沃尔玛 / 美客多 / Shopee / Lazada / 其他 | 每个平台的供应链模式不同，影响库存建模 |
| 各平台主要用哪种发货模式？ | FBA / WFS / 全托管 / 半托管 / 自发货 / 混合 | FBA 需管海外库存天数；全托管卖家无库存数据；半托管需自己管海外仓 |
| 有没有做独立站？ | 有 / 无 | 独立站销量需额外数据源 |

#### 3. 供应链网络（决定仓库和物流参数）

| 提问 | 选项 | 对 PMC 的影响 |
|------|------|------|
| 国内有仓库吗？ | 有，自营 / 有，第三方 / 无国内仓，直发 | 无国内仓则跳过 ods_inventory_domestic 板块 |
| 海外仓分布？ | FBA 美国东/西 / FBA 欧洲 / 第三方海外仓 / 无 | 决定 ods_inventory_overseas 仓库种类；影响场景05 补货的 FC 拆解 |
| 主要物流方式？ | 海运快船 / 海运慢船 / 空运 / 铁路 / 混合 | 影响发货周期参数（P4），快船 15-20 天 vs 慢船 30-45 天 |
| 供应商大概有多少家？ | <5 / 5-20 / >20 | 影响采购数据复杂度 |
| 有没有组合装/套装？ | 有 / 无 | 有则阶段 C 需做前置拆解 |

#### 4. 业务参数预期（动态参数提问）

PMC 系统的规则参数表 `ods_params`（最终展开为 `dwd_params`）包含了所有可配置的参数。每个接入客户可能需要不同的参数值。

**不要硬编码参数列表。** 提取参数的方法是查询数据库：

```sql
-- 查看所有参数及其默认值
SELECT param_no, param_id, param_name, param_default, param_type, param_note
FROM dwd_params
ORDER BY param_no, tier, sub_param;
```

拿到参数表后，逐参数判断是否需要向客户提问：

| 判断条件 | 处理 |
|----------|------|
| 参数默认值是通用合理值（如 weighted_alpha=0.5）且客户无特殊需求 | 不提问，保留默认 |
| 参数依赖客户业务特征（如安全库存天数、采购周期） | **必须提问**，因为不同客户差异巨大 |
| 参数客户已在前面的画像环节给出信息（如物流方式→发货周期） | 基于画像自动推算，不重复提问 |
| 客户主动说「这些用默认就行」 | 跳过剩余参数提问 |

**提问方式**：不要扔参数编号，用自然语言解释这个参数干什么的，然后给出建议范围：

> 「P1 是 A 级货盘的安全库存天数，就是你最核心的爆款需要备多少天库存。我们默认设的是 30 天。你觉得偏保守还是偏激进？或者说你有没有大概的想法？」

**回答记录**：将客户的回答整理成一张参数覆盖表，阶段 D 中写入 ods_params：

```
客户参数覆盖记录

| param_no | param_name | 默认值 | 客户值 | 原因 |
|----------|-----------|--------|--------|------|
| P1 | A类安全库存天数 | 30天 | 45天 | 客户反馈海运周期不稳定，需更保守 |
| P5 | 呆滞判定天数 | 90天 | 60天 | 客户SKU周转快，90天太长 |
| ... | ... | ... | ... | ... |
```

> **重要**：此步骤的目标是收集客户预期，不是当场写入参数。实际写入在阶段 D 完成 ETL 管线搭建之后。

#### 5. 数据系统（决定阶段 D 接入方式）

| 提问 | 选项 | 对 PMC 的影响 |
|------|------|------|
| 用什么 ERP/WMS？ | 万里牛 / 马帮 / 领星 / 积加 / 店小秘 / 自研 / 没用 ERP | 决定能否 API 直连还是走 CSV 导入 |
| 数据能导出吗？是什么格式？ | Excel / CSV / 数据库直连 / API / 手动整理 | 决定阶段 D 管道选型 |
| 谁负责对接数据？ | 运营 / 仓库 / IT / 老板自己 | 建立联系人和沟通节奏 |
